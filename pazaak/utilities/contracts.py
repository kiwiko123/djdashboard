# Inspired by the proposed addition of contracts in C++17.
# 
...
class _ContractCondition(metaclass=abc.ABCMeta):
    def __init__(self, condition: callable, exception=AssertionError, message=''):
...
class expects(_ContractCondition):
    ...
    """
    def __init__(self, condition: callable, exception=AssertionError, message=''):
        super().__init__(condition, exception=exception, message=message)
